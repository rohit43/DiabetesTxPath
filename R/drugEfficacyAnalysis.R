...
drugEfficacyAnalysis <- function(connectionDetails,
  ...
  covariateSettings <- createCovariateSettings(useCovariateDemographics = TRUE,
                                               ...
                                               excludedCovariateConceptIds = c(1529331,
                                                                               1510202,
                                                                               1503297,
                                                                               43013884,
                                                                               40239216,
  ...
  colnames(tPid) <- c("treatmentId", "numPid")
  # Making sure that we should have at-least 250 patients to perform matching. We will not perform the
  # study if the number of patients in T and C is less than 250
  if (tPid$numPid[1] < 250 & tPid$numPid[2] < 250) {
    results <- list()
  } else {
    ...
    if (outCome == 3) {
      ...
      pcTreatmentWithMI <- round(as.numeric(as.character(nrow(subset(studyPopMITreatment,
                                                                     outcomeCount >
        0))/nrow(studyPopMITreatment))), 3)
      pcComparatorWithMI <- round(as.numeric(as.character(nrow(subset(studyPopMIComparator,
                                                                      outcomeCount >
        0))/nrow(studyPopMIComparator))), 3)
      ...
      pcTreatmentWithMIMatched <- round(as.numeric(as.character(nrow(subset(studyPopMITreatment,
                                                                            outcomeCount > 0))/nrow(studyPopMITreatment))), 3)
      pcComparatorWithMIMatched <- round(as.numeric(as.character(nrow(subset(studyPopMIComparator,
                                                                             outcomeCount > 0))/nrow(studyPopMIComparator))), 3)
      MiStat <- data.frame(cbind(pcTreatmentWithMI,
                                 pcComparatorWithMI,
                                 pcTreatmentWithMIMatched,
                                 pcComparatorWithMIMatched))
      colnames(MiStat) <- c("pidFracTreatmentBeforeMatch",
                            "pidFracComparatorBeforeMatch",
                            "pidFracTreatmentAfterMatch",
                            "pidFracComparatorAfterMatch")
      results <- list(psAUC,
                      psScoreBeforeMatching,
                      psScoreAfterMatching,
                      finalAttDiag,
      ...
      pcTreatmentWithKD <- round(as.numeric(as.character(nrow(subset(studyPopKDTreatment,
                                                                     outcomeCount >
        0))/nrow(studyPopKDTreatment))), 3)
      pcComparatorWithKD <- round(as.numeric(as.character(nrow(subset(studyPopKDComparator,
                                                                      outcomeCount >
        0))/nrow(studyPopKDComparator))), 3)
      ...
      pcTreatmentWithKDMatched <- round(as.numeric(as.character(nrow(subset(studyPopKDTreatment,
                                                                            outcomeCount > 0))/nrow(studyPopKDTreatment))), 3)
      pcComparatorWithKDMatched <- round(as.numeric(as.character(nrow(subset(studyPopKDComparator,
                                                                             outcomeCount > 0))/nrow(studyPopKDComparator))), 3)
      KDStat <- data.frame(cbind(pcTreatmentWithKD,
                                 pcComparatorWithKD,
                                 pcTreatmentWithKDMatched,
                                 pcComparatorWithKDMatched))
      colnames(KDStat) <- c("pidFracTreatmentBeforeMatch",
                            "pidFracComparatorBeforeMatch",
                            "pidFracTreatmentAfterMatch",
                            "pidFracComparatorAfterMatch")
      results <- list(psAUC,
      ...
      pcTreatmentWithED <- round(as.numeric(as.character(nrow(subset(studyPopEDTreatment,
                                                                     outcomeCount >
        0))/nrow(studyPopEDTreatment))), 3)
      pcComparatorWithED <- round(as.numeric(as.character(nrow(subset(studyPopEDComparator,
                                                                      outcomeCount >
        0))/nrow(studyPopEDComparator))), 3)
      ...
      pcTreatmentWithEDMatched <- round(as.numeric(as.character(nrow(subset(studyPopEDTreatment,
                                                                            outcomeCount > 0))/nrow(studyPopEDTreatment))), 3)
      pcComparatorWithEDMatched <- round(as.numeric(as.character(nrow(subset(studyPopEDComparator,
                                                                             outcomeCount > 0))/nrow(studyPopEDComparator))), 3)
      EDStat <- data.frame(cbind(pcTreatmentWithED,
                                 pcComparatorWithED,
                                 pcTreatmentWithEDMatched,
                                 pcComparatorWithEDMatched))
      colnames(EDStat) <- c("pidFracTreatmentBeforeMatch",
                            "pidFracComparatorBeforeMatch",
                            "pidFracTreatmentAfterMatch",
                            "pidFracComparatorAfterMatch")
      results <- list(psAUC,
